...
export default function MobileBoard() {
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '50px', color: '#0A2E5C' }}>טוען נתונים...</div>

  const prayers = data?.prayer_times || { weekday: [], shabbat: [], chagim: [] }
  const settings = data?.zmanim_settings || defaultSettings

  return (
    <div style={{ padding: '15px', paddingBottom: '90px', maxWidth: '600px', margin: '0 auto' }}>
      
      {/* HEADER */}
      <header style={{ textAlign: 'center', marginBottom: '20px', backgroundColor: '#FFFFFF', padding: '15px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
        ...
        {shabbatData?.parasha && (
          <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'space-between', fontSize: '1rem', color: '#0A2E5C' }}>
            <span>{shabbatData.parasha.includes('פרשת') ? shabbatData.parasha : `פרשת ${shabbatData.parasha}`}</span>
      ...
      {/* ZMANIM */}
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', padding: '15px', marginBottom: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
        <h2 style={{ color: '#3A6EA5', borderBottom: '2px solid #F0F4F8', paddingBottom: '8px', marginTop: 0, fontSize: '1.4rem' }}>זמני היום</h2>
        <div style={{ fontSize: '1.1rem', lineHeight: '1.8', color: '#0A2E5C' }}>
          ...
          {settings.tzeit && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>צאת הכוכבים:</span> <strong>{formatTime(zmanim.tzeit7023deg || zmanim.tzeit853deg || zmanim.tzeit)}</strong></div>}
        </div>
      </div>
      ...
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', padding: '15px', marginBottom: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
        ...
        {Array.isArray(prayers.weekday) && prayers.weekday.length > 0 && (
          <div style={{ marginBottom: '15px' }}>
            <strong style={{ color: '#0A2E5C', display: 'block', marginBottom: '5px' }}>ימי חול:</strong>
            {prayers.weekday.map((p: any, i: number) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #f9f9f9', padding: '2px 0' }}><span>{p.name}</span> <strong>{p.time}</strong></div>
            ))}
          </div>
        )}

        {Array.isArray(prayers.shabbat) && prayers.shabbat.length > 0 && (
          <div>
            <strong style={{ color: '#0A2E5C', display: 'block', marginBottom: '5px' }}>שבת קודש:</strong>
      ...
      {/* EVENTS */}
      {events.length > 0 && (
        <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', padding: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
